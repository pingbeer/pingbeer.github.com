(function($){
    $.fn.yxMobileSlider = function(settings){
        ...
        var imgList=$("#carouselimgs").val().split(",");
           ...
           //自动加载第一张图片
            ...
            function touchEndFunc(e){
                ...
                if (Math.abs(moveY) < Math.abs(moveX)) {
                    ...
                    oFocus.eq(iCurr).addClass("current").siblings().removeClass("current");
                    $(".curNumhtml").text(Number(iCurr)+1);
                    oLi.eq(iCurr).find("img").attr("src",oLi.eq(iCurr).find("img").data("imgurl"));
                }
            }
            //移动设备基于屏幕宽度设置容器宽高
            function mobileSettings(){
				//10为右边距
                moveWidth = $(window).width()-10;
