...
function doWhite(){
    clientService.doPost({
        ...
        callBackWhenError: "afterDoWhite"
    })
...
function afterDoWhite(returnValue){
 ...
 //   }
	...
	//alert(errorCode+"等于0是正常的强制等于0");
	 errorCode="0";
    if(errorCode == '0'){ //正常        
        status = $(rootXmlNode).find('WhiteResult').find('status').text();
	//	alert(status+"status状态值等于1成功");
	//	alert(verifyTimeFlag == false+"verifyTimeFlag验证这个初始值和false比较结果");
   //   status="1";
        if(status == "0"){ //等待
         //   $('[id="overLayer"]').show();
           ...
           if(verifyTimeFlag == false){
                verifyTimeOut = setTimeout("doWhite()",100);
            }
        }else if(status == "1"){ //成功
            verifyTimeFlag = true;
            $('[id="overLayer"]').hide();
