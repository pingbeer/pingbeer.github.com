...
var flashSaleDetail = {
    ...
    createNew: function () {
        ...
        detail.loadInfo = function () {
            ...
            ajaxJsonProp.servlet=sinoMobileConfig.CONST_SERVLET_FLASH_SALE;
            ajaxJsonProp.act="GET_FLASH_SALE_ITEM_INFO";
        ...
        detail.loadDetailInfoSuccess = function (data) {
            ...
            if (data.code + "" == "1") {
                detail.printInfo(sinoMobileUtil.getJsonObjectFromResponseData(data,"flashSaleItemInfo"));
        ...
        detail.printInfo = function (jsonObj) {
			...
			if(!sinoMobileUtil.strIsNull(jsonObj["saleControl"])){
				saleControlMsg+=","+jsonObj["saleControl"];
			}
			...
			*/
			...
			if( vendorConfigFlg=='1') {
				if(jsonObj["phoneColorsQtyFlash"].indexOf(",") != -1){
					colorArr = jsonObj["phoneColorsQtyFlash"].split(",");
				}else{
					colorArr = jsonObj["phoneColorsQtyFlash"].split(";");
				}
			} else {
           ...
           // $(".colorinput").val(colorArr[0]);//qyj 2016-2-29
            ...
            //qyj 2016-2-24
			if( vendorConfigFlg=='1') {
				var phoneColorsQty=jsonObj["phoneColorsQtyFlash"];
		...
		detail.createOrder = function () {
			...
			*/
            ...
            lineData.addField("promoteApplyLineId",promoteApplyLineId);
		...
		detail.loadCreateOrderSuccess = function (data) {
            if (data.code + "" == "1") {
				...
				if(orderData!=null){
					...
					sinoMobileUI.doOpenCreateOrderPage(poHeaderId,"flashsale");
				}else{
