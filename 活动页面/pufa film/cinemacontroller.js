...
ciname.controller('cinemaController', function ($scope, $rootScope, $http, $state, $cookieStore,
    ...
    $scope.getBanner = function () {
        ...
        var cinemabanner = new Swiper('.cinemaSwiperBanner', {
            pagination: '#swiper-cinema-id',
            paginationClickable: true,
            centeredSlides: true,
            autoplay: 3000,
            autoplayDisableOnInteraction: false,
            onInit: function (cinemabanner) {
                var index = cinemabanner.activeIndex;
                $timeout(function () {
                    $('#swiper-cinema-id span').removeClass('swiper-pagination-bullet-active');
                    $('#swiper-cinema-id span').eq(index).addClass('swiper-pagination-bullet-active');
                }, 1);
            },
            onTransitionEnd: function (cinemabanner) {
                var index = cinemabanner.activeIndex;
                $timeout(function () {
                    $('#swiper-cinema-id span').removeClass('swiper-pagination-bullet-active');
                    $('#swiper-cinema-id span').eq(index).addClass('swiper-pagination-bullet-active');
